...
Board::Board() {
	...
	players_.push_back(new HumanPlayer(this, 0));
	for (int i = 1; i <= 3; i++) {
		players_.push_back(new ComputerPlayer(this, i));
	}
}
...
void Board::Show() {
	...
	for (int i = 0; i < 28; i++) {
		int j = 1;
		if(squares_[i]->GetOwner() == players_[j]->GetName()) {
			p1.push_back(i);
		}
		else if (squares_[i]->GetOwner() == players_[j+1]->GetName()) {
			p2.push_back(i);
		}
		else if (squares_[i]->GetOwner() == players_[j+2]->GetName()) {
			p3.push_back(i);
		}
		else if (squares_[i]->GetOwner() == players_[j+3]->GetName()) {
			p4.push_back(i);
		}
	}
