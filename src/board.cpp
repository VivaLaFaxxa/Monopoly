...
Board::Board(Player* p1,Player* p2,Player* p3,Player* p4) {
	...
	//Initialize the vector of players with the proper type
	players_.push_back(p1); 	//passed by pointers due to command pipe
	p1->SetName("Giocatore 1");
	
	p2 = new ComputerPlayer(this, 2);	
	players_.push_back(p2);
	p2->SetName("Giocatore 2");

	p3 = new ComputerPlayer(this, 3);
	players_.push_back(p3);
	p3->SetName("Giocatore 3");
	
	p4 = new ComputerPlayer(this, 4);
	players_.push_back(p4);	
	p4->SetName("Giocatore 4");
}
...
void Board::MovePlayer(Player& player, int roll) {
	...
	//scrivere in outfile
	std::string s = player.GetName() + " è passato alla casella " + std::to_string(player.GetPosition()) +"\n";
	std::cout << s;
	player.PrintToFile(s);


	//Check if the player has passed the start square
	if (old_position < 0 && new_position >= 0) {			//da controllare qui quando si passa dal via
		player.SetBalance(player.GetBalance() + 20);
		std::cout << "Sei passato dal via! Ritira 20 fiorini \n";
		player.PrintToFile("Sei passato dal via! Ritira 20 fiorini \n");
	}
...
void Board::CheckForWinner() {
	if (players_.size() == 1) {
		std::string s =  "The winner is " + players_[0]->GetName() + "!\n";
		std::cout << s;
		players_[0]->PrintToFile(s);
	}
}
...
void Board::Show() {
	...
	for (int i = side_length * 3; i >= side_length * 2; i--) {
		PrintSquare(i);
	}
	std::cout << std::endl;
}
